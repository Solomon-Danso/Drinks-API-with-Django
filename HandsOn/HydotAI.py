...
stoi = vocab['stoi']  # String to integer mapping
itos = vocab['itos']  # Integer to string mapping
vocab_size = len(stoi)
...
# Function to predict the next token given some input text
def predict(text):
    tokens = text.split()
    vector = torch.zeros(len(tokens), dtype=torch.long)
